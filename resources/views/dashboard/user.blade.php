        ...
        <tr>
            {{-- <th scope="row">{{$i++}}</th> --}}
                ...
                <td class="text-center">
                    <a href="/user/{{ $data->id }}/edit" class="btn btn-primary"> <i class="bi bi-pencil-square"></i></a>
                    <form action="/user/{{ $data->id }}" method="post" class="d-inline">
                        @method('DELETE')
                        @csrf
                        <button class="btn btn-danger"
                        onclick="return confirm('Yang bener di hapus Mas Bro?')"><i class="bi bi-trash-fill"></i></button>
                    </form>
                </td>
        </tr>
        @endforeach
